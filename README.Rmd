...
Data is from `r length(unique(phendat$Clone))` clones in 
`r length(unique(phendat$Orchard))` orchards grown at 
`r length(unique(phendat$Site))` BC seed orchard sites and observed between `r min(phendat$Year, na.rm=TRUE)` 
and `r max(phendat$Year, na.rm=TRUE)`.
